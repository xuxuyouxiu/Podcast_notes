...
### 修复
- **OSS 同步脚本 bytes/str 误用导致 yml 未上传**（v1.52.12/1.52.13 两次 Release 假成功/假失败连环坑的终修）：yml 序列化改为 bytes 后，put_object 处仍对其调用 .encode() → AttributeError，安装包已传但所有 yml 未更新。已修复，并以 mock oss2 对 main() 全流程做断言测试（覆盖 exe/blockmap 上传、绝对 URL 注入、固定路径、历史目录镜像、版本校验）后再发布。
